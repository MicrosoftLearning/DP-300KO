...
9. 데이터베이스 설정 화면에서 Azure SQL 데이터베이스 버전을 범용으로 변경합니다. 서비스 목표를 **GP_Gen5_2**로 변경하고 다음을 클릭합니다. [스크린샷에 나와 있는 **GP_Gen4_2**는 2020년 8월 이후 제공되지 않습니다.]
...
12. 텍스트를 쿼리 창에 붙여 넣고 192.168.1.1.을 4단계의 클라이언트 IP 주소로 바꿔 다음 T-SQL 쿼리를 실행합니다. 실행을 클릭하거나 F5 키를 누릅니다.
	...
	다음으로 AdventureWorks 데이터베이스에 포함된 사용자를 만듭니다. 새 쿼리를 클릭하고 다음 T-SQL을 실행합니다. AdventureWorks 데이터베이스를 여전히 사용하고 있는지 확인합니다. 아래 강조 표시된 상자에 마스터가 표시되면 아래로 당겨서 AdventureWorks로 전환할 수 있습니다.
...
1. Azure Portal로 이동하여 화면 오른쪽 상단 모서리에 있는 사용자 이름을 클릭합니다.
	...
	**참고:** Azure SQL Database를 사용하는 Azure Active Directory 관리자의 경우 Microsoft 계정(예: Outlook, Gmail, Hotmail 또는 Yahoo의 사용자 계정)이 지원되지 않습니다. 이에 대한 해결책으로, DBA라는 Azure Active Directory 그룹을 만들고 이 그룹에 사용자 계정을 추가하면 됩니다. 또는 작업 2를 건너뛰어도 됩니다.
