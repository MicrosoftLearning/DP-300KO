...
1. 랩 가상 머신에서 브라우저 세션을 시작하고 [https://portal.azure.com](https://portal.azure.com/)으로 이동합니다. 

	![그림 1](../images/dp-3300-module-11-lab-01.png)

 

	Azure Portal에서 화면 상단의 검색 창에 구독을 입력합니다. 구독 아이콘을 클릭합니다.
...
4. 모든 리소스 페이지에는 선택한 구독 내에 모든 Azure 리소스가 나열됩니다. 리소스 목록에 기존 가상 머신이 있어야 합니다. 
...
5. 왼쪽 탐색에서 리소스 그룹을 선택합니다.
	...
	리소스 그룹 블레이드에는 선택한 구독 내에 모든 Azure 리소스 그룹이 나열됩니다. 페이지에 나열된 기존 리소스 그룹이 있어야 합니다. 
...
7. 랩에 적합한 구독을 선택했는지 확인합니다. 리소스 그룹 DP-300-Lab01의 이름을 지정합니다. 사용자 위치에서 가장 가까운 Azure 지역을 선택하고 검토 + 만들기 단추를 선택합니다. 그런 다음, 만들기 단추를 선택합니다. 
	...
	![그림 12](../images/dp-3300-module-11-lab-08.png)

8. 페이지의 상단에 있는 검색 창을 찾습니다. Azure SQL을 검색합니다. 서비스 아래 결과에 표시되는 Azure SQL에 대한 검색 결과를 선택합니다.
...
2. SQL 배포 옵션 선택 블레이드에서 SQL 가상 머신 아래의 드롭다운 상자를 엽니다. SQL Server 무료 라이선스 레이블이 지정된 옵션을 선택합니다. Windows Server 2019의 SQL 2019 Developer 그런 다음, 만들기 단추를 선택합니다.
	...
	![그림 9](../images/dp-3300-module-11-lab-11.png)
...
3. 가상 머신 만들기 블레이드에 다음 정보를 입력합니다. 
	...
	- 크기: 표준 **D2s v3** (vCPU 2개, 8GiB 메모리. 이 옵션을 표시하려면 "크기" 단추를 선택해야 할 수 있음)
	...
	후속 랩 작업에 사용할 수 있으므로 나중에 사용하기 위해 사용자 이름과 암호를 기록합니다.
...
4. 디스크 탭으로 이동하여 구성을 검토합니다. 
	...
	![그림 15](../images/dp-3300-module-11-lab-13.png)

 
5. 네트워킹 탭으로 이동하여 구성을 검토합니다. 

	![그림 16](../images/dp-3300-module-11-lab-14.png)
 

6. 관리 탭으로 이동하여 구성을 검토합니다. 

	![그림 19](../images/dp-3300-module-11-lab-15.png)

	여기에서 만드는 스토리지 계정 이름을 기록하여 이후 랩에서 사용합니다. "자동 종료 사용"이 꺼져 있는지 확인합니다.


7. 고급 탭으로 이동하여 구성을 검토합니다. 

	![그림 20](../images/dp-3300-module-11-lab-16.png)


8. SQL Server 설정 탭으로 이동하여 구성을 검토합니다. 

	![그림 21](../images/dp-3300-module-11-lab-17.png)
...
​
	![그림 22](../images/dp-3300-module-11-lab-18.png)
...
1. 가상 머신의 개요 페이지에서 연결 단추를 선택하고 RDP를 선택합니다. 
	...
	![그림 26](../images/dp-3300-module-11-lab-21.png)
...
7. 원격 데스크톱 창이 열립니다. 오른쪽에서 아니요 단추를 선택하여 네트워크의 다른 디바이스에서 컴퓨터를 검색하기를 원치 않음을 표시합니다. 상자 오른쪽 상단에 있는 X를 클릭하여 서버 관리자 대화 상자를 닫습니다. 
...
​
	![그림 39](../images/dp-3300-module-11-lab-34.png)
...
13. 실행 단추를 선택하거나 F5 키를 눌러 쿼리를 실행합니다. 2010년 매출액이 **43421.0364**인지 확인합니다. 
...
**dp300sqlvmlab01** Virtual Machine의 주 블레이드로 이동하여 메뉴에서 삭제 단추를 클릭합니다. 
