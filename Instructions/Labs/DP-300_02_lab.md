---
lab:
    title: '랩 2 – Azure SQL Database 배포'
    module: '데이터 플랫폼 리소스 계획 및 구현'
---

# 랩 2 – Azure SQL Database 배포
...
3. 기존 리소스 그룹을 검토한 다음 **추가** 버튼을 클릭하여 새 리소스 그룹을 만듭니다.  
​
...
​
	![사진 6](images/dp-3300-module-22-lab-04.png)랩

2. 다음 정보를 이용해 기본 페이지를 작성합니다.
	...
	- 지역: 리소스 그룹이 만들어졌을 때 동일한 지역을 선택합니다(해당 위치와 가장 가까운 지역).  
​
...
3. 네트워크의 IP 범위를 구성하고 Azure SQL 데이터베이스 엔드포인트에 대해 준비합니다.
...
2. 상단의 검색 상자에서 "SQL 데이터베이스"를 검색한 다음 옵션 목록에서 **SQL Database**를 클릭합니다.  
​
	![그림 15](images/dp-3300-module-22-lab-10.png)

3. **만들기** 단추를 클릭합니다.

4. 다음 입력으로 SQL Database 기본 화면 만들기를 완료한 후 **다음: 네트워킹을 클릭합니다.**
	...
	- 서버: **새로 만들기**를 클릭합니다. 새 서버 사이드바에서 다음과 같이 양식을 작성합니다.
		...
		- 새 서버 사이드바는 아래와 같이 보여야 합니다. **확인**을 클릭합니다.
...
1. SQL Database 블레이드에서 상단 섹션의 SQL Server 이름에 대한 링크를 클릭합니다.  
​
...
## 작업 1: Azure Data Studio에서 Azure SQL DB 인스턴스를 등록합니다.
...
2. Azure Data Studio가 열리면 아래와 같이 왼쪽 메뉴의 확장 단추를 클릭합니다.

 
	![자동으로 생성된 휴대폰 설명의 스크린샷](images/dp-3300-module-22-lab-23.png)

 

3. 확장자 검색 상자에 Postgres를 입력합니다. Microsoft PostgreSQL 확장자가 표시됩니다. 아래 강조 표시된 설치 단추를 클릭합니다.

	![자동으로 생성된 휴대폰 설명의 스크린샷](images/dp-3300-module-22-lab-24.png)

 

4. ADS의 왼쪽 사이드바에서 **연결** 단추를 클릭한 다음 **연결 추가** 단추를 클릭합니다.
	
	![그림 30](images/dp-3300-module-22-lab-25.png)

5. 연결 사이드바에서 연결 세부 정보 섹션을 연결 정보와 함께 작성하여 이전 연습에서 만든 SQL Database에 연결합니다.

	- 연결 형식: **Microsoft SQL Server**

	- 서버: 연습 2, 작업 1에서 만든 SQL Server의 이름을 입력합니다. 예를 들면 다음과 같습니다. **dp300-lab-xx.database.windows.net**  
	...
	- 데이터베이스 드롭다운을 펼쳐 **AdventureWorksLT**를 선택합니다. 이 서버에 클라이언트 IP를 추가하기 위해 방화벽 규칙을 추가하라는 메시지가 표시되면 계정 추가를 클릭하고 Azure 계정에 로그인합니다. 새 방화벽 만들기 화면에서 **확인**을 클릭합니다.
...
6. ADS는 데이터베이스에 연결하고 DB에 대한 몇 가지 기본 정보와 개체의 일부 목록을 표시합니다.  
​
...
12. Azure Data Studio 내에서 파일 메뉴의 Notebook을 D:\Labfiles\Deploy Azure SQL Database(이 폴더는 VM에 이미 있음) 디렉터리에 저장합니다. Azure Data Studio 내부에서 Notebook 탭을 닫습니다. 파일 메뉴에서 Notebook 열기를 선택하고 방금 저장한 Notebook을 엽니다. Notebook에 있는 쿼리와 함께 쿼리 결과가 저장되었는지 관찰합니다.
...
2. Azure 서비스에 대한 액세스 허용을 **예**로 밉니다.  
​
	![그림 50](images/dp-3300-module-22-lab-40.png)

3. 왼쪽 상단에서 **저장**을 클릭합니다.

## 작업 3: Azure Data Studio를 사용하여 PostgreSQL 데이터베이스에 연결

**참고:** PostgreSQL 확장이 ADS에 설치되면 PostgreSQL을 쿼리할 수 있습니다. 이 확장은 랩 VM에 사전 설치됩니다.
...
3. 연결 세부 정보 사이드바에서 다음 정보를 입력하여 이전 작업의 PostgreSQL 데이터베이스에 연결합니다.
	...
	- 서버: 작업 1에서 만든 PostgreSQL Server의 이름을 입력합니다. 예를 들면 다음과 같습니다. **dp300-lab02-doc.database.windows.net** (포털의 개요 창에서 서버의 전체 이름을 찾을 수 있음).

	- 인증 형식: **암호**

	- 사용자 이름: **dp300admin@dp300-lab02-doc** (사용자 이름에 호스트 이름이 포함되어야 함). 
...
6. PostgreSQL 데이터베이스에 대한 ADS의 연결을 기록합니다.  
​
