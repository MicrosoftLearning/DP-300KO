...
2. **+ 추가**를 클릭하여 **가상 네트워크 만들기** 페이지를 엽니다. **기본** 탭에서 다음 작업을 완료합니다.
...
2. Azure Data Studio가 열리면 ADS의 왼쪽 사이드바에서 **연결** 단추를 클릭한 다음 **연결 추가** 단추를 클릭합니다.
...
3. 연결 사이드바에서 연결 세부 정보 섹션을 연결 정보와 함께 작성하여 이전 연습에서 만든 SQL Database에 연결합니다.
	...
	- 서버: 연습 2, 작업 1에서 만든 SQL Server의 이름을 입력합니다. 예를 들어 다음과 같은 이름을 입력할 수 있습니다. **dp300-lab-xx.database.windows.net**  
...
4. ADS는 데이터베이스에 연결하고 DB에 대한 몇 가지 기본 정보와 개체의 일부 목록을 표시합니다.  
‎
...
3. 연결 세부 정보 사이드바에서 다음 정보를 입력하여 이전 작업의 PostgreSQL 데이터베이스에 연결합니다.
	...
	- 서버: 작업 1에서 만든 PostgreSQL Server의 이름을 입력합니다. 예를 들어 다음과 같은 이름을 입력할 수 있습니다. **dp300-lab02-doc.database.windows.net**(포털의 개요 창에서 서버의 전체 이름을 찾을 수 있음).

	- 인증 형식: **암호**

	- 사용자 이름: **dp300admin@dp300-lab02-doc**(사용자 이름에는 위에서 지정한 호스트 이름이 포함되어야 함. 'doc' 대신 자체 접두사 사용) 
