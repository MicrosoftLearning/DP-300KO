...
**필수 구성 요소**: 유지 관리를 위해 Azure database AdventureWorks의 SQL 로그인이 만들어졌습니다.
...
Azure Portal의 오른쪽 상단에 있는 클라우드 셸 아이콘을 클릭합니다.
...
학생들은 학습과 사례 연구에서 얻은 정보를 사용하여 AdventureWorks 내 자동화 프로세스를 구성하고 구현합니다. 

# 랩 목표

이 랩을 완료하면 다음을 수행할 수 있습니다:

1. GitHub 빠른 시작 템플릿에서 Azure 리소스 배포

2. 유지 관리 작업 예약 및 알림 구성
...
6. Azure SQL Database에서 인덱스 유지 관리를 수행하려면 Azure Automation Runbook 배포하세요.
...
3. Azure SQL Database에서 인덱스 유지 관리를 수행하려면 Azure Automation Runbook 배포하세요.
...
2. 위의 링크로 다시 이동하여 Azure에 배포 단추를 클릭합니다. Azure Portal에 로그인하라는 메시지가 표시될 수 있습니다. 제공된 자격 증명으로 로그인합니다.


3. 다음 화면이 표시됩니다. 이 템플릿을 배포하려면 빈 필드를 작성해야 합니다.


    ![자동으로 생성된 휴대폰 설명의 스크린샷](../images/dp-3300-module-66-lab-06.png) 

    리소스 그룹 필드에서 "새로 만들기"를 클릭하고 DP300-Lab06을 입력합니다. SQL 관리자 로그인 필드에서 **labadmin**을 입력합니다. 암호 필드에 암호 **Azur3Pa$$** 를 입력합니다. "사용 약관에 동의함" 옆의 확인란을 클릭한 다음 구매를 클릭합니다. 


4. 구매를 클릭한 후. 배포가 시작됩니다. 벨(아래 스크린샷에서 강조 표시됨)을 클릭한 다음, 알림 창에서 진행 중인 배포 링크를 클릭하여 배포 상태를 추적할 수 있습니다.
...
5. 완료되면 화면이 새로 만든 리소스에 대한 링크로 업데이트됩니다. 
    ...
    리소스로 이동 링크를 클릭합니다. 배포를 만들면 Azure 리소스 그룹으로 이동합니다. 다음 연습에서 사용하므로 SQL 서버 이름을 기록해둡니다.
...
## 작업 1: CPU가 평균 80%를 초과할 때 경고를 생성합니다.

1. portal.azure.com으로 다시 이동합니다. 다시 로그인해야 할 수도 있습니다. Azure Portal 상단의 검색 창에서 SQL을 입력하고 SQL 데이터베이스를 클릭합니다. 데이터베이스 이름을 클릭합니다.
...
5. 리소스가 만든 데이터베이스로 이미 채워져 있음을 알 수 있습니다. 조건 섹션에서 조건 선택을 클릭합니다.

    ![자동 생성된 소셜 미디어 게시물 설명의 스크린샷](../images/dp-3300-module-66-lab-14.png)

6. 신호 로직 구성 슬라이드 아웃 메뉴에서 CPU 백분율을 선택합니다. 
...
8. 작업 그룹(선택 사항) 섹션에서 작업 그룹 선택을 클릭합니다. 
...
9. 작업 그룹에 대한 플라이 아웃에서 작업 그룹 만들기를 클릭합니다.
    ...
    작업 그룹 화면에서 작업 그룹 이름 필드에 emailgroup을 입력합니다.
...
11. 이메일/SMS 메시지/푸시/음성을 선택하고 DemoLab이라는 이름을 입력합니다. 오른쪽의 플라이아웃 화면에서 이메일 옆의 확인란을 클릭하고 확인을 클릭합니다. 그리고 검토 및 만들기 단추를 클릭한 다음, 작업 그룹 만들기 화면에서 만들기를 클릭합니다.


    경고 규칙 만들기 화면에서 DemoAlert라는 경고 규칙 이름을 추가한 다음, 아래와 같이 경고 규칙 만들기를 클릭합니다.
...
1. 유지 관리 작업 스크립트를 배포합니다.

2. Azure SQL Database에서 인덱스 유지 관리를 수행하려면 Azure Automation Runbook 배포하세요.

## 작업 1: Azure SQL Database에서 인덱스를 다시 빌드하려면 Automation Runbook을 배포합니다.
...
3. Automation 계정 만들기
    ...
    리소스 그룹: 랩 2에서 만든 리소스 그룹을 사용합니다. **DP300-Lab02**

    위치: 랩 2에서 Azure SQL 서버를 만든 지역 사용

    만들기를 클릭합니다. 자동화 계정은 1~3분 안에 배포해야 합니다. 
...
4. AdaptativeIndexDefragmentation의 **github** 페이지 [https://github.com/microsoft/tigertoolbox/blob/master/AdaptiveIndexDefrag/usp_AdaptiveIndexDefrag.sql](https://github.com/microsoft/tigertoolbox/blob/master/AdaptiveIndexDefrag/usp_AdaptiveIndexDefrag.sql)로 이동합니다.
    ...
    원시를 클릭합니다. 이렇게 하면 복사할 수 있는 형식으로 코드를 제공합니다. 모든 텍스트를 선택하고 클립보드에 복사합니다. 
...
6. 데이터베이스에 로그인하기 위한 자격 증명을 묻는 메시지가 표시됩니다. 연습 1에서 만든 자격 증명을 사용합니다.
    ...
    위와 같이 오류 메시지 끝에 있는 파란색 서버 방화벽 링크 설정을 클릭합니다. 서버 방화벽 화면이 나타납니다. 아래에 표시된 대로 클라이언트 IP 추가를 클릭합니다.


    ![자동으로 생성된 휴대폰 설명의 스크린샷](../images/dp-3300-module-66-lab-30.png)

    저장 단추를 클릭하여 방화벽 구성을 저장합니다.
    ...
    쿼리 편집기로 돌아가면 확인을 클릭하여 데이터베이스에 로그인합니다. 2단계에서 복사한 텍스트를 쿼리 1 창에 붙여넣습니다. 

    ![자동 생성된 소셜 미디어 게시물 설명의 스크린샷](../images/dp-3300-module-66-lab-32.png)


    위의 스크린샷에서 강조 표시된 쿼리의 5번째 및 6번째 줄에서 USE msdb 및 GO 문을 삭제한 다음, 실행을 클릭합니다. 

    ![자동 생성된 소셜 미디어 게시물 설명의 스크린샷](../images/dp-3300-module-66-lab-33.png)


    쿼리를 완료하는 데 20초 정도 걸립니다. 


7. Azure Portal에 들어가면 왼쪽 메뉴를 확장하고 기본 Azure 패널 내의 모든 서비스를 클릭합니다. 
...
14. 데이터베이스에 안전하게 로그인하려면 자격 증명을 만들어야 합니다. 자동화 계정 블레이드에서 공유 리소스 섹션으로 이동하여 자격 증명을 클릭합니다.
    ...
    + 자격 증명 추가를 클릭한 다음, 아래와 같이 자격 증명을 만듭니다. 자격 증명 SQLUser 이름을 지정하고 사용자 이름에 labadmin, 암호에 Azur3Pa$$를 사용합니다. 만들기를 클릭합니다.
...
16. IndexMaintenance의 Runbook 이름과 Powershell의 Runbook 유형을 제공합니다. 선택한 짧은 설명을 제공할 수 있습니다. 완료되면 [만들기]를 클릭합니다. 
...
17. 아래 코드를 Runbook에 복사합니다. Azure SQL 서버의 이름을 반영하도록 $AzureSQLServerName 변수를 편집합니다. 아래와 같이 Azure SQL Database의 개요 화면에서 이 값을 얻을 수 있습니다.

    ![자동으로 생성된 휴대폰 설명의 스크린샷](../images/dp-3300-module-66-lab-46.png)


    .database.windows.net 도메인 접미사가 포함된 서버 이름을 복사합니다. Windows Explorer를 사용하여 D:\LabFIles\Automate Resources 폴더에서 DP300-Lab-Module-6.ps1 파일 [열기]를 마우스 오른쪽 버튼으로 클릭하고 [편집]을 클릭합니다. PowerShell IDE가 열립니다.
...
18. 자동화 Runbook으로 돌아가 17단계에서 PowerShell IDE에서 복사한 PowerShell 코드에 붙여 넣습니다. 그런 다음, [저장]을 클릭하고 [게시]를 클릭합니다.

    ![그림 679031903](../images/dp-3300-module-66-lab-48.png)


19. 이전에 게시된 버전을 덮어쓰라는 메시지가 표시되면 예를 클릭합니다. 

    ![그림 36](../images/dp-3300-module-66-lab-49.png)

20. 다음으로 정기적으로 실행되도록 Runbook을 예약합니다. 왼쪽 탐색 메뉴에서 일정을 클릭합니다. 그런 다음, 상단에 있는 일정 추가를 클릭합니다. 
...
24. 동부 표준시로 다음 날 오전 4:00를 시작 시간으로 지정합니다. 1일마다 재발생을 구성합니다. 만료를 설정하지 마십시오. 
